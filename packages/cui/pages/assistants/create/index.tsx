...
const AssistantCreate = () => {
	...
	const generateConfig = async (
		...
		const steps: Array<{ id: string; title: string; description?: string; delay: number }> = [
			...
			},
			{
				...
				description: is_cn ? '创建专家基础配置' : 'Creating expert base configuration',
				delay: 1000
			},
			{
				id: 'generate_prompt',
			...
			},
			{
				...
				description: is_cn ? '匹配合适的专家和工具' : 'Matching appropriate experts and tools',
				delay: 800
			},
			{
				id: 'complete',
				title: is_cn ? '完成' : 'Completed',
				description: is_cn ? '专家配置已生成' : 'Expert configuration generated',
				delay: 500
			}
		]
	...
	const handleCreate = async () => {
		...
		try {
			...
			message.error(is_cn ? '创建专家失败' : 'Failed to create expert')
		} finally {
			setCreating(false)
		}
	}
		...
		<div className={styles.container}>
			<div className={styles.breadcrumbContainer}>
				<Breadcrumb>
					...
					<Breadcrumb.Item>{is_cn ? '创建专家' : 'Create Expert'}</Breadcrumb.Item>
				</Breadcrumb>
			</div>
			...
			{currentStep === 0.5 ? (
				...
				<div className={styles.heroSection}>
					<div className={styles.heroContent}>
						<h1 className={styles.heroTitle}>{is_cn ? '创建专家' : 'Create Expert'}</h1>
						<p className={styles.heroDesc}>
							{is_cn
								? '描述你的需求，AI 将为你创建专家'
								: 'Describe your needs, AI will create an expert for you'}
						</p>
						<div className={styles.chatBoxWrapper}>
							<ChatBox
								schema={{
									type: 'string',
									placeholder: is_cn
										? '例如：我需要一个客服专家，能够回答产品相关问题，处理用户投诉...'
										: 'e.g., I need a customer service expert that can answer product questions, handle complaints...'
								}}
								value={requirement}
