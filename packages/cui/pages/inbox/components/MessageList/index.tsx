...
const CATEGORY_LABELS: Record<InboxCategory, { cn: string; en: string }> = {
	all: { cn: '全部', en: 'All' },
	bookmarked: { cn: '收藏', en: 'Bookmarked' },
...
function getGroupIcon(group: InboxGroup): { icon: string; color: string } {
	...
	if (m.type === 'completed') {
		return {
			...
			color: hasUnread ? 'var(--color_success)' : 'var(--color_neo_text_tertiary)'
		}
	}
	if (m.type === 'failed') {
		return {
			icon: 'material-assignment_late',
			color: hasUnread ? 'var(--color_warning)' : 'var(--color_neo_text_tertiary)'
		}
	}
