...
import type { OCRProviderPreset, OCRProviderConfig } from '../../types'
import type { SearchProviderField } from '@/openapi/setting/types'
...
export default function OCRProviderCard({ preset, config, onToggle, onSave, onReload }: OCRProviderCardProps) {
	...
	const allFilled = preset.fields.every((f) => f.optional || fieldValues[f.key]?.trim())
	...
	const handleSave = async () => {
		...
		try {
			...
			const testResp = await api.TestOCRProvider(preset.key, { field_values: submitValues })
	...
	const handleTest = async () => {
		...
		try {
			...
			const resp = await api.TestOCRProvider(preset.key, { field_values: testValues })
			...
			{(preset.description || preset.website) && (
				<div className={styles.searchCardDesc}>
					...
					{preset.website && (
						...
						>
							{preset.website?.includes('github.com')
								? (is_cn ? '项目主页 →' : 'GitHub →')
